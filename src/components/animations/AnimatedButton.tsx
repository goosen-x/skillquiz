  ...
  return (
    <motion.div {...animationProps} className="inline-block">
      <Button
        ...
        {...props}
      >
        <motion.span className="relative z-10" initial={{ opacity: 1 }} whileHover={{ opacity: 1 }}>
          {children}
        </motion.span>
...
export function FloatingActionButton({
  ...
  return (
    <motion.button
      className={cn(
        'fixed bottom-8 right-8 w-14 h-14 rounded-full text-white shadow-2xl z-50 flex items-center justify-center cursor-pointer',
        variants[variant],
        className
      )}
