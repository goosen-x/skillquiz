...
export type CalculateResultFunction = (answers: (string | number)[] | Record<string, string | number>) => UniversalTestResult;
...
// Маппинг всех тестов
const testImportMap: Record<string, () => Promise<any>> = {
...
function adaptQuestions(questions: any[], testSlug: string): UniversalQuestion[] {
  ...
  if (questions[0]?.question && questions[0]?.icon) {
  ...
  if (questions[0]?.text) {
    return questions.map(q => ({
      ...
      options: q.options.map((opt: any, optIndex: number) => ({
        text: opt.text,
        value: opt.score ?? opt.value ?? opt.points ?? opt.type ?? opt.style ?? opt.language ?? opt.temperament ?? 
               // Для attachment-style и других тестов с множественными баллами используем индекс
               (opt.secure !== undefined || opt.anxious !== undefined || opt.avoidant !== undefined || opt.disorganized !== undefined 
                ? optIndex.toString() 
    ...
    if (testSlug === 'temperament' || testSlug === 'love-language' || 
      ...
      if (Array.isArray(answers)) {
        ...
        answers.forEach((answer, index) => {
          if (answer != null) { // Проверяем на null/undefined
      ...
      adaptedAnswers = answers.filter(a => a != null).map(a => {
...
function adaptResult(result: any, testSlug: string): UniversalTestResult {
  // Базовая адаптация
  const adapted: UniversalTestResult = {
    id: result.id || result.type || result.level || result.primary_type || result.primary_language || '',
  ...
  console.log('Test slug:', testSlug, 'Original color:', result.color, 'Adapted color:', adapted.color);
...
function generateChartData(scores: Record<string, number>, testSlug: string): any[] {
...
export async function loadTest(test: TestData): Promise<LoadedTestData> {
  ...
  const calculateKey = Object.keys(testModule).find(key => 
    key.startsWith('calculate') && (key.endsWith('Result') || key.endsWith('Results'))
  );
  ...
  return {
    ...
    results: results ? results.map((r: any) => adaptResult(r, test.slug)) : undefined,
  };
}
