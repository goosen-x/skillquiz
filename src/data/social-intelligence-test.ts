...
export interface SocialIntelligenceQuestion {
  id: string;
  text: string;
  category: 'emotion_reading' | 'social_cues' | 'empathy' | 'communication' | 'relationship_management' | 'social_problem_solving';
  scenario?: string;
  options: {
    text: string;
...
export const calculateSocialIntelligenceResult = (answers: Record<string, number>): SocialIntelligenceResult => {
  ...
  let social_skills: string[];
  let communication_tips: string[];
  let relationship_advice: string[];
  let professional_benefits: string[];
  let development_exercises: string[];
  let warning_signs: string[];
  let social_situations_guide: string[];

  if (total_score <= 44) {
    overall_level = 'low';
    title = 'Низкий социальный интеллект';
    description = 'Вам может быть сложно понимать социальные ситуации, читать эмоции других людей и эффективно взаимодействовать в группе. Это нормально, и эти навыки можно развить с практикой и осознанными усилиями.';
    strengths = [
    ...
    description = 'У вас есть базовые социальные навыки, но иногда возникают трудности в понимании более сложных социальных ситуаций. Целенаправленная работа над развитием эмоционального интеллекта принесет хорошие результаты.';
    strengths = [
    ...
    description = 'У вас нормальный уровень социальных навыков. Вы способны эффективно взаимодействовать с большинством людей в обычных ситуациях, но можете испытывать затруднения в более сложных или напряженных социальных контекстах.';
    strengths = [
    ...
    description = 'У вас хорошо развитые социальные навыки. Вы легко понимаете эмоции других людей, эффективно общаетесь и способны успешно навигировать в сложных социальных ситуациях. Люди часто чувствуют себя комфортно в вашем обществе.';
    strengths = [
    ...
    description = 'У вас исключительно высокий уровень социального интеллекта. Вы интуитивно понимаете людей, мастерски читаете социальные ситуации и способны эффективно влиять на групповую динамику. Вы естественный лидер и медиатор.';
    strengths = [
