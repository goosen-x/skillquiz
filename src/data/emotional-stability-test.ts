...
export interface EmotionalStabilityQuestion {
  id: string;
  text: string;
  category: 'stress_response' | 'emotional_regulation' | 'resilience' | 'anxiety_management' | 'mood_stability';
  options: {
    text: string;
...
export const calculateEmotionalStabilityResult = (answers: Record<string, number>): EmotionalStabilityResult => {
  ...
  let support_systems: string[];

  if (total_score <= 40) {
    overall_level = 'low';
    title = 'Низкая эмоциональная стабильность';
    description = 'У вас низкий уровень эмоциональной стабильности. Вы часто испытываете сильные эмоциональные реакции на стресс, сложно справляетесь с тревогой и имеете склонность к перепадам настроения. Это может значительно влиять на качество жизни и отношения.';
    strengths = [
    ...
    description = 'Ваша эмоциональная стабильность ниже среднего. Вы довольно часто испытываете стресс и эмоциональные колебания, но еще способны справляться с повседневными вызовами. Работа над эмоциональными навыками поможет улучшить качество жизни.';
    strengths = [
    ...
    description = 'У вас средний уровень эмоциональной стабильности. Вы в целом справляетесь с жизненными вызовами, но иногда можете испытывать значительный стресс. В некоторых ситуациях вам может потребоваться дополнительная поддержка.';
    strengths = [
    ...
    description = 'У вас высокий уровень эмоциональной стабильности. Вы хорошо справляетесь со стрессом, быстро восстанавливаетесь после неудач и поддерживаете стабильное эмоциональное состояние в большинстве ситуаций.';
    strengths = [
    ...
    description = 'У вас очень высокий уровень эмоциональной стабильности. Вы демонстрируете исключительную способность сохранять спокойствие под давлением, быстро адаптируетесь к изменениям и поддерживаете позитивный настрой даже в сложных ситуациях.';
    strengths = [
  ...
  if (overall_level === 'low' || overall_level === 'below_average') {
    ...
    professional_help = [
      ...
      'Обучение менторству и поддержке других'
    ];
    lifestyle_recommendations = [
